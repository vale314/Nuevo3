...
import SignUpForm from '../../../componets/Expolitec/Proyectos/RegistroA.jsx';
...
class SignUpPage extends React.Component {
   ...
   */
   processForm(event) {
     ...
     xhr.open('put', '/auth/registropcar');
   ...
   componentWillMount() {
     const xhr = new XMLHttpRequest();
     xhr.open('get', '/proye/carreras');
     xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
     // set the authorization HTTP header
     xhr.setRequestHeader('Authorization', `bearers ${localStorage.getItem('proye')}`);
     xhr.responseType = 'json';
     xhr.addEventListener('load', () => {
       ...
       if (xhr.status === 200) {
         if(xhr.response.carreras.length !== 0){
            ...
            this.props.router.replace('/registrop')
          }
